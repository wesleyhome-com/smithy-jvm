    ...
    private fun generateJdkHttpTransport(packageName: String): JavaFile {
        ...
        val typeBuilder = TypeSpec.classBuilder("JdkHttpTransport")
            ...
            .addMethod(MethodSpec.methodBuilder("execute")
                ...
                .addStatement("\$T.Builder builder = \$T.newBuilder().uri(\$T.create(request.uri()))", httpRequest, httpRequest, uri)
                .addStatement("builder.method(request.method(), \$T.BodyPublishers.ofByteArray(request.body()))", httpRequest)
                .addCode("\n")
                ...
                .addStatement("\$T<byte[]> response = client.send(builder.build(), \$T.ofByteArray())", httpResponse, bodyHandlers)
    ...
    private fun generateOkHttpTransport(packageName: String): JavaFile {
        ...
        val typeBuilder = TypeSpec.classBuilder("OkHttpTransport")
            ...
            .addMethod(MethodSpec.methodBuilder("execute")
                ...
                .returns(ClassName.get(packageName, "HttpResponse"))
                .addException(ClassName.get("java.io", "IOException"))
                .addStatement("\$T.Builder builder = new \$T.Builder().url(request.uri())", okHttpRequest, okHttpRequest)
                .addCode("\n")
                .addStatement("\$T body = (request.body() == null || request.body().length == 0) ? null : \$T.create(request.body(), \$T.parse(\"application/json\"))", 
    ...
    private fun generateHttpRequest(packageName: String): JavaFile {
        val typeBuilder = TypeSpec.recordBuilder("HttpRequest")
            ...
            .recordConstructor(MethodSpec.constructorBuilder()
                ...
                .addParameter(ParameterizedTypeName.get(ClassName.get("java.util", "Map"), 
                    ClassName.get("java.lang", "String"), 
                    ParameterizedTypeName.get(ClassName.get("java.util", "List"), ClassName.get("java.lang", "String"))), "headers")
